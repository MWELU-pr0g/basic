...
{
    return static::findOne(['contact' => $contact]);
}
...
{
    $model = Yii::$app->db->createCommand('SELECT * FROM user WHERE id=:id')->bindValue('id',$id)->queryOne();
    
    return $model;

    // print_r($model);exit;

    $data =  new DataArrayDataProvider(
        [
            'models' => $model,
        ]
    );
